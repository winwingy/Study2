...
应用程序向导已为您创建了此 HelloATL 项目，作为编写动态链接库 (DLL) 
的起点。
...
HelloATL.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(HelloATL.h)
        GUID 定义			(HelloATL_i.c)
        类型库				(HelloATL.tlb)
        封送处理代码			(HelloATL_p.c 和 
                                           dlldata.c）

HelloATL.h
    此文件包含 HelloATL.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 HelloATL.pch 的预编译头 (PCH) 文件和
